...
const getOneVehiculo = async (req, res) =>{
    ...
    const Vehiculos = await vehiculoservice.getOneVehiculo({ where: { nombre_veh }})
    res.json(Vehiculos)
}

const updateVehiculo = async (req, res) =>{
    const id_veh = req.params.id
    const Vehiculos = await vehiculoservice.updateVehiculo(req.body,{
        where:{
            id_veh
        }
    })
...
const deleteVehiculo = async (req, res) =>{
    const id_veh = req.params.id
    const Vehiculos = await vehiculoservice.deleteVehiculo({ where:{ id_veh }})
    res.json(Vehiculos)
}
